...
For SSR routes, check the session in an async server component so unauthenticated users are redirected before any HTML is streamed — no flash of unauthenticated UI. Build the core `sessionOptions(auth, { headers })` descriptor, then cross the React Query boundary with `adaptServerQueryOptions` from `@better-auth-ui/react/server`. That adapter is intentionally framework-local: core owns the shared descriptor, while React owns `queryOptions`/`DataTag` typing. Call `ensureServerQuery` with a per-request `QueryClient` (direct `auth.api` call, no HTTP hop), and wrap the rendered children in `HydrationBoundary` so downstream `useSession` calls read from the hydrated cache under `authQueryKeys.session` on mount.
...
Because `ensureServerQuery` populates the query cache during SSR and `HydrationBoundary` ships the dehydrated state to the client, child components calling `useSession` render synchronously without a loading state.
