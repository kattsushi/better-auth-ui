...
Session server-auth helpers are canonical from `@better-auth-ui/core/server` as `sessionOptionsServer`, `ensureSessionServer`, `prefetchSessionServer`, and `fetchSessionServer`. The React server entrypoint also keeps compatibility aliases (`sessionOptions`, `ensureSession`, `prefetchSession`, `fetchSession`) and mirrors supported non-session read helpers with server-auth signatures. These helpers accept your Better Auth server instance (`auth`) and request params instead of a browser `authClient`.
