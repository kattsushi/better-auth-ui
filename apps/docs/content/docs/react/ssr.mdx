...
If you'd rather skip the HTTP round-trip and hit the Better Auth server directly from a TanStack Start server function (or any other server runtime), build the descriptor from `@better-auth-ui/core/server` and adapt it with `@better-auth-ui/react/server`. Core owns endpoint descriptors; the React server entrypoint owns React Query adapter helpers.
...
2. Wrap it with `adaptServerQueryOptions(...)` from `@better-auth-ui/react/server`.
3. Pass the adapted options to `ensureServerQuery`, `prefetchServerQuery`, or `fetchServerQuery` from the same React server entrypoint.

This replaces the removed endpoint-specific framework wrappers with one descriptor-plus-adapter pattern. Cache keys are still identical between server descriptors and client hooks, so data prefetched on the server hydrates cleanly into client-side hooks like `useSession`, `useListApiKeys`, and `useActiveOrganization`.
