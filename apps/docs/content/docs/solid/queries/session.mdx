...
const session = await ensureServerQuery(
  queryClient,
  adaptServerQueryOptions(sessionOptions(auth, { headers: request.headers }))
)
```
...
<auto-type-table path="../../../../../../packages/core/src/server/queries/auth/session-query.ts" name="SessionParams" />
