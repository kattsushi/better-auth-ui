...
The Solid server entrypoint also provides matching server-auth helpers for settings, API-key, passkey, multi-session, and organization reads, so server-prefetched data hydrates into the same cache keys used by client hooks.
