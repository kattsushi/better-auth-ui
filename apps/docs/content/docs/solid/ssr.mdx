...
If you'd rather skip the HTTP round-trip and hit the Better Auth server directly from a TanStack Start server function (or any other server runtime), import the server helpers from `@better-auth-ui/solid/server`. They accept your Better Auth server instance instead of an `authClient`:
...
The Solid server entrypoint mirrors the React server helper families with Solid Query adapter types. True server-auth helpers require your app's Better Auth instance and request headers, while client-side route-loader helpers still live in `@better-auth-ui/solid`.

Available `@better-auth-ui/solid/server` helper families include:
...
- Server auth types: `AuthServer`, `ApiKeyAuthServer`, `MagicLinkAuthServer`, `MultiSessionAuthServer`, `OrganizationAuthServer`, `PasskeyAuthServer`, `UsernameAuthServer`

Cache keys are identical between server helpers and component hooks, so data prefetched in route loaders hydrates cleanly into client-side hooks like `useSession`, `useListApiKeys`, and `useActiveOrganization`.
