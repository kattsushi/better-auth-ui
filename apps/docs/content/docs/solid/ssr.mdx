...
Session server-auth helpers are canonical from `@better-auth-ui/core/server` as `sessionOptionsServer`, `ensureSessionServer`, `prefetchSessionServer`, and `fetchSessionServer`. The Solid server entrypoint also keeps compatibility aliases (`sessionOptions`, `ensureSession`, `prefetchSession`, `fetchSession`) and mirrors React's supported non-session read helpers with server-auth signatures.

Available `@better-auth-ui/solid/server` exports include:

- Session compatibility helpers: `sessionOptions`, `ensureSession`, `prefetchSession`, `fetchSession`
