...
The returned `{ session }` is available to child routes and components via `Route.useRouteContext()`. Because the server `ensureServerQuery` call populates the query cache during SSR, downstream `useSession` calls can reuse the hydrated session.
